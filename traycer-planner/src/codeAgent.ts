...
import { FileSystemTools, SearchResult } from './tools.js';
...
export class CodeSearchAgent {
    ...
    private model: any; // The actual model instance
    private history: any[];
    public tools: FileSystemTools;
    private isIndexed: boolean = false;

    constructor(options?: { tools?: FileSystemTools; systemPrompt?: string }) {
...
- Next steps: specific actions
    ...
    private async resolveContextFiles(input: string): Promise<string> {
...
\`\`\`\n`;
    ...
    private async generateResponseWithTools(): Promise<string> {
        try {
            ...
            switch (name) {
                ...
                case 'listDirectory':
                    return await this.tools.listDirectory(
                        parameters.dirPath ?? parameters.path,
                        parameters.recursive || false
                    );

                case 'searchFile':
                    return await this.tools.searchFile(
                        parameters.pattern,
                        parameters.rootDir || '.'
                    );
                ...
                case 'searchCodebase':
                    return await this.tools.searchCodebase(
                        parameters.query,
                        parameters.limit || 10
                    );

                case 'getFileInfo':
                    return await this.tools.getFileInfo(
    ...
    public async indexCodebase(rootDir: string = '.'): Promise<void> {
    ...
    public async grepSearch(term: string, filePattern: string = '**/*'): Promise<SearchResult[]> {
