...
export class FileSystemTools {
     ...
     */
    async searchFile(pattern, rootDir = '.') {
        try {
     ...
     */
    async grep(searchTerm, rootDir = '.', filePattern = '**/*') {
        try {
            const fullRootPath = path.resolve(rootDir);
     ...
     */
    splitCodeIntoChunks(filePath, content) {
        const lines = content.split('\n');
        const chunks = [];
        ...
        try {
            ...
            if (this.embeddings.size > 0) {
                ...
                for (const [key, embedding] of this.embeddings) {
                    ...
                    const chunk = this.codeChunks.find(c => c.filePath === filePath && c.startLine === startLine);
                    if (chunk) {
